...
| Playlist/album resolution | Bot-side `SpotifyClient` (Authorization Code OAuth, own token — not LavaSrc) fetches the track list fast, then lazily resolves each track to audio (Deezer `dzsearch:` first, falling back to YouTube `ytsearch:` if Deezer has no match or errors) as it enters a 5-track lookahead window. Needed because Spotify now blocks playlist/album reads for client-credentials (app-only) tokens — LavaSrc's only workaround does a slow extra HTTP call per track. This bot-side title/artist search is separate from LavaSrc's own `providers` chain (which only resolves Spotify/Apple/Deezer *links*), so it has to redo the Deezer-then-YouTube fallback itself. Single-track links still go straight through LavaSrc as before. Search calls pass `source=None` to `wavelink.Playable.search()` — the query already carries its search prefix, and wavelink prepends its own default prefix (`ytmsearch:`) on top of any non-URL query otherwise, producing an unresolvable double-prefixed identifier. |
