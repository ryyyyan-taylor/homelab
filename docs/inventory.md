...
| Lavalink plugins | LavaSrc `4.8.3` — Spotify source (single-track link resolution, client-credentials flow) and `ytdlp` source (actual audio via yt-dlp binary). `dev.lavalink.youtube` (youtube-plugin) removed 2026-08-19 — every client (WEB, TV/OAuth, MWEB, ANDROID, IOS, etc.) failed for every track (bot detection / SABR-only formats / an open upstream bug) |
