...
- Known gotcha | **Proxmox host kernel slab leak (io_uring / CVE-2026-23259)**: Confirmed 2026-08-07. `SUnreclaim` grows linearly at ~270–400 MB/h, unreclaimable, does not self-heal. Root cause is the kernel io_uring iovec cleanup leak ([CVE-2026-23259](https://nvd.nist.gov/vuln/detail/cve-2026-23259)) — `io_req_rw_cleanup()` can exit with an allocated iovec that fails insertion into `rw_cache` and is never freed. Fired by QEMU's `aio=io_uring` on the Talos VM disks; each leaked allocation is 16 KB via `iovec_from_user` → `__io_import_iovec` → `io_submit_sqes`. **Fix: `aio = "threads"` on all VM disks** (in `terraform/talos-vms.tf`; `aio=native` is not an option because the disks use `cache=writeback`, which needs O_DIRECT). Takes effect only on VM stop/start. The upstream fix landed ~kernel 6.18, but this host is pinned to `6.14.11-9-pve` for the NVIDIA driver (see GPU passthrough section), so the kernel cannot simply be upgraded.
  - Diagnosis technique: the leak is **invisible in `/proc/slabinfo`** (only ~1.2 GB of 8.5 GB accounted) because allocations >8 KB bypass slab caches via `__kmalloc_large_node` and go to the page allocator, still charged to `SUnreclaim`. Identify the caller with ftrace: enable `events/kmem/kmalloc` with filter `bytes_req > 8192` plus `options/stacktrace`.
  - Note: an earlier (2026-05) diagnosis attributed this to Flannel/kube-proxy restart churn. That was **not** the driver — the leak grows at a constant rate with zero pod restarts.
  - Symptom: `free -h` shows near-zero available memory despite low RSS in `ps`. OOM killer fires on KVM processes once swap saturates. Recovery for already-leaked memory: reboot the host (nothing else reclaims it). Detection: PrometheusRule `ProxmoxHostSlabLeakWarning` fires at 4 GB SUnreclaim; `ProxmoxHostSlabLeakCritical` at 10 GB.
...
| 27 | spotify-tool-config | `spotify-tool` | SOPS secrets (Spotify creds, session secret) |
| 28 | spotify-tool | `spotify-tool` | Spotify playlist/library manager (web + downloader) at `spotify.lab.ryantaylor.tech` |
...
| Lavalink plugins | LavaSrc `4.8.3` — Spotify source (single-track link resolution, client-credentials flow), Deezer source (primary audio, see below), and `ytdlp` source (fallback audio via yt-dlp binary). `dev.lavalink.youtube` (youtube-plugin) removed 2026-08-19 — every client (WEB, TV/OAuth, MWEB, ANDROID, IOS, etc.) failed for every track (bot detection / SABR-only formats / an open upstream bug) |
| Lavalink plugin cache | `lavalink-plugins` PVC (`local-path`, 200Mi) mounted at `/opt/Lavalink/plugins`; `fetch-plugins` initContainer seeds it from the plugins' GitHub release assets (idempotent, skips files that already exist) instead of relying on `maven.lavalink.dev` at every pod restart |
| yt-dlp binary | Fetched fresh into an `emptyDir` on every Lavalink pod start (`fetch-ytdlp` initContainer) — must be the standalone `yt-dlp_linux` (PyInstaller) build, not the plain `yt-dlp` zipapp; `ghcr.io/lavalink-devs/lavalink:4` is a JRE image with no `python3`, so the zipapp can't execute |
| Known gotcha | 2026-08-08 host reboot: lavalink crash-looped because it had no volume for `/opt/Lavalink/plugins`, so it re-downloaded both plugin jars from `maven.lavalink.dev` on every restart — that host was down, and the liveness probe killed the pod mid-download before it could bind port 2333. Fixed by adding the PVC + GitHub-sourced `fetch-plugins` initContainer above. Separately, plugin classloading itself stalled for 2+ minutes on one worker node in the first few minutes after the cluster-wide reboot (reproduced with a debug pod; ruled out disk speed — the same PVC read at 2GB/s once isolated — most likely scheduler/JIT contention from every pod on the node restarting at once) and cleared on its own; the relaxed liveness `initialDelaySeconds`/`failureThreshold` above gives it enough room to ride that out instead of getting killed mid-boot |
| Audio source | Deezer primary (`dzisrc:%ISRC%` → `dzsearch:%QUERY%`), YouTube (`ytdlp` search) as fallback when Deezer has no match. Deezer fixed the "wrong song" problem — ISRC-matched studio track instead of a YouTube search result that could land on a music video/lyric video/cover. Reverse-engineered, not officially supported by Deezer or distributed by LavaSrc: needs a `masterDecryptionKey` (not published anywhere in this repo or by LavaSrc — sourced externally) and an `arl` session cookie tied to an actual Deezer account, entered manually via `sops edit music-bot-secret.sops.yaml` (`deezer-master-decryption-key`, `deezer-arl` keys) — never committed in plaintext. Deliberately capped at `formats: [MP3_128, MP3_64]` (free-tier quality) so a free/burner account works — no Premium subscription tied to the account that ends up flagged. Use a burner account, not a personal one: the `arl` is a real login session and can be banned for automated/bot-like use; there is no way to make this fully safe, only lower the cost of losing the account. `arl` cookies also expire (observed ~90 days) and need periodic replacement |
...
| Playlist/album resolution | Bot-side `SpotifyClient` (Authorization Code OAuth, own token — not LavaSrc) fetches the track list fast, then lazily resolves each track to audio as it enters a 5-track lookahead window. Needed because Spotify now blocks playlist/album reads for client-credentials (app-only) tokens — LavaSrc's only workaround does a slow extra HTTP call per track. This bot-side title/artist search is separate from LavaSrc's own `providers` chain (which only resolves Spotify/Apple/Deezer *links*), so it redoes the Deezer-then-YouTube fallback itself: playlist tracks carry an ISRC from Spotify (`external_ids`, free — same API call, no extra request) and resolve via exact-match `dzisrc:` first; album tracks (Spotify's simplified track objects have no `external_ids`) and any ISRC miss fall through to `dzsearch:`/`ytsearch:` text search, scanning the first 5 candidates and skipping ones that look like a remix/cover/karaoke/live version (title regex + normalized title/artist match) before falling back to YouTube, then to the first raw result if nothing looks clean. Single-track links still go straight through LavaSrc as before, so they only get the configmap's `dzisrc:%ISRC%`/`dzsearch:%QUERY%` provider chain, not this filtering. Search calls pass `source=None` to `wavelink.Playable.search()` — the query already carries its search prefix, and wavelink prepends its own default prefix (`ytmsearch:`) on top of any non-URL query otherwise, producing an unresolvable double-prefixed identifier. |
| Secrets | `music-bot/music-bot-secret` (SOPS: `music-bot-config/music-bot-secret.sops.yaml`) — `lavalink-password`, `spotify-client-id`, `spotify-client-secret`, `spotify-refresh-token`, `discord-bot-token`, `discord-guild-id`, `deezer-arl`, `deezer-master-decryption-key` |
...
| | |
|---|---|
| Namespace | `spotify-tool` |
| URL | `https://spotify.lab.ryantaylor.tech` |
| Source | `apps/spotify-tool/` — moved in from the standalone `~/Code/spotify-tool` repo (kept, unmodified, as a historical mirror) |
| Purpose | Custom Spotify library manager: artist-playlist ↔ liked-songs two-way sync, a combined-playlist builder, library browsing, and MP3 playlist downloads |
| Architecture | Two Deployments, one ArgoCD app (same split as `music-bot`'s bot+Lavalink): `web` (Node/Express API + built React frontend, single container/image) and `downloader` (Python/spotdl/ffmpeg, internal-only), so the ffmpeg/spotdl dependency chain doesn't bloat the always-on web image |
| `web` | Image `ghcr.io/ryyyyan-taylor/homelab-spotify-tool-web`, built by `.github/workflows/spotify-tool.yaml`. Serves the built frontend and the `/api/*` Express routes from one process/pod. SQLite (session/token storage only — playlists are not cached locally) on a 512Mi `local-path` PVC at `/app/data` |
| `downloader` | Image `ghcr.io/ryyyyan-taylor/homelab-spotify-tool-downloader`, same workflow. Internal ClusterIP Service only (`downloader.spotify-tool.svc.cluster.local:8000`, no IngressRoute) — `web` proxies download requests to it and streams the resulting zip back to the browser. No PVC; downloads live in the pod's ephemeral `/tmp` and are cleaned up after 1 hour, same as the original single-host implementation. `spotdl` needs a writable `$HOME` for its `~/.spotdl` cache — the container runs as `nobody`, whose default home isn't writable, so the Dockerfile sets `HOME=/tmp` |
| Secrets | `spotify-tool/spotify-tool-secret` (SOPS: `spotify-tool-config/spotify-tool-secret.sops.yaml`) — `spotify-client-id`, `spotify-client-secret`, `session-secret` |
| Spotify app | Reuses the same grandfathered pre-2024 Spotify app as `music-bot` (see that section above) rather than registering a new one. Needs `https://spotify.lab.ryantaylor.tech/api/auth/callback` added as a redirect URI on that app in the Spotify Developer Dashboard (manual, one-time) |
| Resources (web) | req `cpu 25m / mem 64Mi`, lim `cpu 300m / mem 256Mi` — provisional |
| Resources (downloader) | req `cpu 100m / mem 128Mi`, lim `cpu 1 / mem 512Mi` — provisional, sized for ffmpeg transcoding rather than idle |
